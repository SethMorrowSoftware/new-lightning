...
final class Rest
{
    ...
    public static function fetch(): array
    {
        ...
        [$body, $status, $error] = self::request($url, $headers);
        ...
        foreach ($list as $item) {
            ...
            $records[] = [
                'lat' => $lat,
                'lon' => $lon,
                'ts' => Ingest::parseTimestamp(Ingest::dotPath($item, $timePath), $timeFormat),
            ];
        }
        ...
        return [
            ...
            'sample' => $list[0] ?? null,
            'message' => sprintf(
                'Mapped %d strike%s from the response%s.',
                count($records),
                count($records) === 1 ? '' : 's',
     ...
     * @return array{0:string,1:int,2:string|null} body, status, error
     */
    private static function request(string $url, array $headers): array
    {
        if (function_exists('curl_init')) {
            ...
            curl_close($ch);
            return [is_string($body) ? substr($body, 0, self::MAX_BODY_BYTES) : '', $status, $error];
        }
        ...
        if ($body === false) {
            return ['', $status, 'The request could not be completed.'];
        }
